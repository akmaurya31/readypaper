...
where tbl_create_pepar.teacher_id= $id");
	...
	$query = $this->db->query("SELECT *
                                ...
                                where tbl_capter_question_paper.pepar_id=$pepar_id and finalSaveStatus=1");
		return $query->row();
	}

	public function get_draft_teacher_pepar($empId,$pepar_id){
		
	$query = $this->db->query("SELECT *
                                FROM `tbl_capter_question_paper`
                                where tbl_capter_question_paper.empId=$empId and tbl_capter_question_paper.pepar_id=$pepar_id and tbl_capter_question_paper.saveAsDraft=1");
		return $query->result();
	}
