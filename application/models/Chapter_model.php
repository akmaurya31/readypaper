...
class Chapter_model extends CI_Model
{
	...
	function get_datatables($role,$id,$class_sub_id)
	{
		...
		if($role=='admin'){
		    $this->db->where('clas_sub_id',$class_sub_id);
		}else{
		    $this->db->where('clas_sub_id',$class_sub_id);//->where('created_by',$id)
		}
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function get_by_name($name, $id = null)
	{
		$this->db->from($this->table);
		if (is_null($id)) {
			$this->db->where('chapter_name', $name);
		} else {
			$this->db->where('chapter_name', $name);
			$this->db->where('id !=', $id);
		}
	...
	public function get_active_chapter()
	{
		$this->db->from($this->table);
	...
	public function get_active_class_subject($class_sub_id)
	{
		$this->db->from($this->table);
		$this->db->where('clas_sub_id', $class_sub_id);
		$query = $this->db->get();
