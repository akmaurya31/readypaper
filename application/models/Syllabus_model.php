...
class Syllabus_model extends CI_Model
{
	...
	var $table = 'tbl_syllabus';
	var $column_order = array('syllabus_name'); //set column field database for datatable orderable
	var $column_search = array('syllabus_name'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	...
	function get_datatables($role,$id,$class_sub_id)
	{
		...
		if($role=='admin'){
		    ...
		    $this->db->where('created_by',$id)->where('class_sub_id',$class_sub_id);
		}
		$query = $this->db->get();
		return $query->result();
	}
