...
<div class="container-fluid">
    <div class="row page-titles">
        <ol class="breadcrumb">
          ...
          <button type="button" class="btn btn-default  btn-xs waves-effect waves-light">No Punch (P)</button>
        </ol>
    </div>
    <!-- row -->
     ...
     <div class="card shadow mb-4">
                     <div class="card-header py-3">
                        <div class="form-group col-sm-8">
								<label for="inputEmail3" class="col-sm-6 control-label">Teacher & Employee:</label>
								<div class="col-sm-6">
								    <select class="form-control" name="location"  onchange ="getDepartmentID(this.value)">
								         <option value="">Select Employee:</option>
								        <?php foreach($getdepartments as $val){?>
								           <option value="<?= $val->id?>"<?php  if(@$_GET['depid']==$val->id){ echo "selected" ;}  ?> ><?= $val->firstName.' ('.$val->role.')'?></option>
								        <?php } ?>
								    </select>
                     ...
                     <div class="card-body">
                        <form  class="form-horizontal" method="post">
                         <div class="row">
							<div class="col-sm-1"> <a href="attendanceRegister?date=<?= date("Y-m",strtotime(date("Y-m", strtotime($_GET['date'])) . " -1 month"))  ?>&&depid=<?php echo $_GET['depid'] ;?>" class="btn btn-info" >Previous</a></div>  
							<div class="col-sm-2 text-center"> <?= date("F-Y", strtotime($_GET['date'])); ?></div>
							<div class="col-sm-1"><a href="attendanceRegister?date=<?= date("Y-m",strtotime(date("Y-m", strtotime($_GET['date'])) . "+1 month")) ?>&&depid=<?php echo $_GET['depid'] ;?>" class="btn btn-info">Next</a></div>
					     </div>
                        </form>
                         <br>
...
<div class="modal fade" id="myModalStatus" role="dialog">
  ...
  <tbody id="showtable"></tbody>
       ...
       function getIpaddress(dates,day,empid){
        ...
        $.ajax({
            type: "post",
            url: "<?php echo base_url();?>attendanceRegister/getIpaddress",
            data:{'dates':dates ,
                  'day':day,
                  'empid':empid
            },            
          ...
          function getDepartmentID(depid){
                window.location.href='attendanceRegister?depid='+depid+'&date='+'<?php  if(isset($_GET['date'])){ echo date("Y-m");}else{ echo date("Y-m");} ?>'; 
