...
class Admin_class_subject extends MY_Controller {
    ...
    public function index() {
        $data['include'] = 'admin/class_subject/class_subject';
    ...
    //echo $this->db->last_query();die;
        ...
        foreach ($list as $class_subject) {
           ...
           if ($class_subject->status == 'Y')
            ...
            $row[] = @$className->class_name.' ('.$class_subject->id.')';; 
 ...
 $output = array(
    ...
    public function class_subject_add() {
        ...
        $count = count($this->input->post('subject_id'));
          ...
          $subjectNameid = $this->input->post('subject_id');
        $insert_data = array();
        for ($i = 0; $i < $count; $i++) {
             $insert_data[] = array(
            'class_id' => $this->input->post('class_id'),
            'board_id' => $this->input->post('board_id'),
            'subject_id' => $subjectNameid[$i],
