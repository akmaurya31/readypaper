...
class Admin_school extends MY_Controller {
    ...
    public function ajax_list() {
        ...
        foreach ($list as $school) {
            ...
            if ($school->pic)
            $row[] = '<img src="' . base_url('upload/admin-school/' . $school->pic) . '" class="img-responsive" style="width:100%;"/>';
        else
            $row[] = '(No photo)';
 ...
 $output = array(
    ...
    public function ajax_update() {
        ...
        if ($this->input->post('remove_photo')) { // if remove photo checked
            if (file_exists('upload/admin-school/' . $this->input->post('remove_photo')) && $this->input->post('remove_photo'))
                unlink('upload/admin-school/' . $this->input->post('remove_photo'));
            $data['pic'] = '';
        }
        ...
        if (!empty($_FILES['pic']['name'])) {
            ...
            if (file_exists('upload/admin-school/' . $school->pic) && $school->pic)
                unlink('upload/admin-school/' . $school->pic);
            $data['pic'] = $upload;
        }
    ...
    private function _do_upload() {
        $config['upload_path'] = './upload/admin-school/'; //where is file upload path
    ...
    private function _validate() {
        ...
        if ($this->input->post('head') == '') {
            ...
            $data['error_string'][] = 'School is required';
            $data['status'] = FALSE;
        }
